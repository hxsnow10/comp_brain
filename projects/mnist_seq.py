...
def get_seq_data(sample):
    batch_size,seq_size = sample.shape
    rval = [sample[:, k] for k in range(seq_size)]
    return rval

def main():
    ds_train, ds_test = get_pytorch_minist_data()
    train_env = SimpleSupervisedEnv(ds_train, sample_split = get_seq_data)
    test_env = SimpleSupervisedEnv(ds_test, sample_split = get_seq_data)
    model_names = ["Bp-da"]
